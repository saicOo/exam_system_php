...
public function store($request){
    ...
    */
    ...
    if(empty($sub_id)) $this->messErrors[] = "the input subject is empty";
    /********************************************
    *** check inputs length
    */
    if(strlen($title) > 50) $this->messErrors[] = "the name max length char 50";     
    // #### Check for errors ####
    if(!filter_var($total_question, FILTER_VALIDATE_INT)) $this->messErrors[] = "the input total question is not integer";     
...
########################     next question     ########
    public function saveAnswarNext($exam_id,$page,$ques_id,$option){
        ...
        header("location:/start_exam.php?ref=".$exam_id.'&page='.$page);
        exit;
    }
###############################################################
########################     Prev question     ########
